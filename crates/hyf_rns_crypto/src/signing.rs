...
use crate::identity::ed25519_verifying_key;
use crate::{RnsCryptoError, RnsPublicIdentity, RnsSecretIdentity};

pub fn sign(identity: &RnsSecretIdentity, message: &[u8]) -> Result<[u8; 64], RnsCryptoError> {
...
pub fn verify(
    ...
    let verifying_key = ed25519_verifying_key(identity)?;
    let signature = Signature::from_bytes(signature);
    verifying_key
        .verify(message, &signature)
        .map_err(|_| RnsCryptoError::InvalidSignature)
}
...
mod tests {
    ...
    const OTHER_SECRET_IDENTITY_BYTES: [u8; 64] = [
